...
async function fakeApiCall(search: string): Promise<string[]> {
  await wait(2e3);
  return ['Option 1', 'Option 2', 'Country 1', 'Country 2'].filter(
    (it) => it.toLowerCase().indexOf(search.toLowerCase()) > -1
  );
}

async function fakeApiCallWithObjects(
  search: string
): Promise<DVKComboBoxFieldValue[]> {
  await wait(2e3);
  return [
...
async function fakeApiCallWithDifferentObjects(
  search: string
): Promise<DVKComboBoxFieldValue[]> {
  await wait(2e3);
  return [
    ...
    (it) =>
      it.field1.toLowerCase().indexOf(search.toLowerCase()) > -1 ||
      it.field2.toLowerCase().indexOf(search.toLowerCase()) > -1
  );
}
...
const ComboBoxSection: FC = () => {
  ...
  const [submittedDefaultedMultiple, submitDefaultedMultiple] = useState<
    DVKObject
  >();
  const [changedDefaultedMultiple, changeDefaultedMultiple] = useState<
    DVKObject
  >();
  const [submittedDefaultedModifiers, submitDefaultedModifiers] = useState<
    DVKObject
  >();
  const [changedDefaultedModifiers, changeDefaultedModifiers] = useState<
    DVKObject
  >();
