  ...
  return (
    <Accordion defaultExpanded {...accordionProps}>
          ...
          <Grid item className={classes.expansionPanelTable}>
            {values.length ? (
              <DVKTable
                ...
                columns={
                  fields.filter((field) => !isHiddenField(field)) as DVKColumn[]
                }
                actions={actions}
                rows={values}
      ...
      <InputModal
        ...
        onCreate={(newValue: DVKListItem) => {
          updatePropertyF(name, (oldValues) =>
            (oldValues as DVKListItem[]).map((value) =>
              isEqual(value, newValue) ? newValue : value
            )
          );
          closeEditModal();
        }}
      ...
      <ConfirmationModal
        ...
        onAccept={() => {
          updatePropertyF(name, (oldValues) =>
            (oldValues as DVKListItem[]).filter(
              (value) => !isEqual(value, deleteModalData)
            )
          );
          closeDeleteModal();
        }}
