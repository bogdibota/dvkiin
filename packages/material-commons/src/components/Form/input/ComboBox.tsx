...
import {
  ...
  PropsWithErrorManagement,
} from '../domain';

function filterDuplicates(
  options: DVKComboBoxFieldValue[]
): DVKComboBoxFieldValue[] {
...
const InputComboBox: FunctionComponent<
  DVKComboBoxField & PropsWithErrorManagement
> = ({
  name,
  label,
  type,
  ...
  const [options, setOptions] = useState<DVKComboBoxFieldValue[]>(
    staticOptions || []
  );
  ...
  const getOptionSelected = (
    option: DVKComboBoxFieldValue,
    value: DVKComboBoxFieldValue
  ) => (option.name ? option.name === value.name : option === value);

  const multipleValue = multiple
    ? (
        lett(deepGet(obj, name, []), (names: string[]) =>
          names.map((name) =>
            options.find((opt) => opt.name === name || opt === name)
          )
        ) || []
      ).filter((value) =>
  ...
  return (
    ...
    >
      <Autocomplete
        ...
        onChange={(
          _: ChangeEvent,
          newValue: DVKComboBoxFieldValue | null | DVKComboBoxFieldValue[]
        ) => {
          updateProperty(
            name,
            type
            ...
            multiple
              ? newValue.map(
                  (value: DVKComboBoxFieldValue) => value.name || value
                )
              : newValue
          );
        }}
