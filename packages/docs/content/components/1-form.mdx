...
<PropsTable>
  ...
  <tr>
    ...
    <td>
      {'({ id }) =>'}{' '}
      <inlineCode style={noWrap}>{"`Edit '${ id }'`"}</inlineCode>{' '}
    </td>
  ...
  <tr>
    ...
    <td>
      {'({ id }) =>'}{' '}
      <inlineCode style={noWrap}>{"`Delete '${ id }'`"}</inlineCode>{' '}
    </td>
...
<PropsTable>
  ...
  <tr>
    ...
    <td>false</td>
    <td>
      If true, the field will have multiple lines (like a `textarea`). Only
      considered if the type is `text`.
    </td>
  </tr>
  <WithErrorManagement />
</PropsTable>
...
<DVKForm
  fields={[{ name: 'avatar', label: 'Avatar', type: 'image' }]}
  onSubmit={handleSubmit}
/>;
```
